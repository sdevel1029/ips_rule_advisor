...
async def test(request: Request, client=Depends(get_supabase_client)):
    ...
    if os.path.isfile("normal_pcap/" + "/" + user_id + "/" + cve + ".pcap"):
        ...
        ans["total"] = (
            test_result["공격 패킷 결과"]["총 패킷 수"]
            + test_result["정상 패킷 결과"]["총 패킷 수"]
        )
        ans["attacknum"] = test_result["공격 패킷 결과"]["총 패킷 수"]
        ans["normalnum"] = test_result["정상 패킷 결과"]["총 패킷 수"]
        ans["accuracyrate"] = (
            test_result["정상 패킷 결과"]["오탐된 패킷 수"]
            + test_result["공격 패킷 결과"]["미탐된 패킷 수"]
        ) / ans["total"]
        ans["attackrate"] = (
            test_result["정상 패킷 결과"]["오탐된 패킷 수"] / ans["normalnum"]
        )
        ans["normalrate"] = (
            test_result["공격 패킷 결과"]["미탐된 패킷 수"] / ans["attacknum"]
        )
        ans["attacktrue"] = (
            ans["attacknum"] - test_result["공격 패킷 결과"]["미탐된 패킷 수"]
        ) / ans["attacknum"]
        ans["normaltrue"] = (
            ans["normalnum"] - test_result["정상 패킷 결과"]["오탐된 패킷 수"]
        ) / ans["normalnum"]
        ans["normallatency"] = (
            test_result["정상 패킷 결과"]["평균 시간_룰 적용 후"]
            - test_result["정상 패킷 결과"]["평균 시간_룰 적용 전"]
        )
        ans["normalcpu_usage"] = (
            test_result["정상 패킷 결과"]["평균 cpu_룰 적용 후"]
            - test_result["정상 패킷 결과"]["평균 cpu_룰 적용 전"]
        )
        ans["normalmemory_usage"] = (
            test_result["정상 패킷 결과"]["평균 memory_룰 적용 후"]
            - test_result["정상 패킷 결과"]["평균 memory_룰 적용 전"]
        )
        ans["attacklatency"] = (
            test_result["공격 패킷 결과"]["평균 시간_룰 적용 후"]
            - test_result["공격 패킷 결과"]["평균 시간_룰 적용 전"]
        )
        ans["attackcpu_usage"] = (
            test_result["공격 패킷 결과"]["평균 cpu_룰 적용 후"]
            - test_result["공격 패킷 결과"]["평균 cpu_룰 적용 전"]
        )
        ans["attackmemory_usage"] = (
            test_result["공격 패킷 결과"]["평균 memory_룰 적용 후"]
            - test_result["공격 패킷 결과"]["평균 memory_룰 적용 전"]
        )

    return templates.TemplateResponse(
        "ruletest_result.html", {"request": request, "test_result": ans}
    )
